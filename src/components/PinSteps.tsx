    ...
    <div className="grid grid-cols-[0.8fr_1.2fr] items-start gap-12 max-[900px]:grid-cols-1">
      ...
      <div className="flex flex-col">
        {steps.map((step, i) => (
          ...
          >
            <span className="font-display text-[13px] font-extrabold tracking-[0.1em] text-sv-primary-light">STEP {String(i + 1).padStart(2, '0')}</span>
            ...
            <ul className="mt-4 flex flex-col gap-2">
              {step.items.map((item) => (
                <li key={item} className="relative pl-5 text-[15px] text-sv-text-muted before:absolute before:left-0 before:content-['—'] before:text-sv-primary-light">
                  {item}
                </li>
              ))}
            </ul>
